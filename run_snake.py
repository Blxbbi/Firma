...
async def main_logic():
    ...
    VerificationRegistry.register("scope", ScopeVerifierAdapter)
    ...
    # Phase 2 (Idee A) Step3.3: if an existing project folder is configured, ingest it
    # into the workspace and register the ingest context (writes baseline_manifest.json).
    # Must run AFTER create_run (needs the real run_id).
    from engine.services.ingest import setup_ingest_run
    setup_ingest_run(run_id)
    ...
    if FIRMA_TRANSPORT == "pimesh":
        ...
        receiver = PiMeshReceiverLoop(
            ...
            task_done_events=_task_done_events,
        )
...
def _select_config():
    ...
    if app == "counter":
        ...
        cfg = _snake_config()
    # Optional override: a caller-provided prompt (e.g. an incremental edit for the
    # Loop-Smoke). Falls back to the app's built-in prompt when unset.
    custom = os.environ.get("FIRMA_PROMPT")
    if custom:
        cfg = dict(cfg)
        cfg["prompt"] = custom
    return cfg
