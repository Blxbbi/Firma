    ...
    def _build_spec_markdown(self, payload: Dict[str, Any], crew_cwd: str, review_artifacts: Optional[Dict[str, str]] = None) -> str:
        ...
        if role == "PLANNER":
            parts.append(
                ...
                "**Delta-Erkennung (WICHTIG):**\n"
                "- Wenn der Research Brief von einer Aenderung an einem bestehenden Projekt spricht\n"
                "  (z.B. 'Contact-Sektion hinzufuegen', 'Button-Farbe aendern'),\n"
                "  dann plane nur diese Aenderung (1 Task, oder falls noetig 2-3 Tasks).\n"
                "- Wenn der Research Brief ein neues Projekt beschreibt,\n"
                "  dann plane die vollstaendige Umsetzung (3-10 Tasks je nach Komplexitaet).\n"
                "- Analysiere den Research Brief sorgfaeltig: Ist es eine Aenderung oder ein Neubau?\n\n"
            )
            if research_brief:
            ...
            parts.append(
                ...
                "- **Wenn das Projekt bereits Dateien enthält** (Ingest Mode mit bestehendem Projekt):\n"
                "  - Beginne `research/brief.md` NICHT mit dem vollen User Goal\n"
                "  - Beschreibe stattdessen das **Delta**: Was fehlt, was muss geaendert werden\n"
                "  - Der Planner soll nur die Aenderung umsetzen, nicht das ganze Projekt neu bauen\n"
                "- Wenn der User eine Website will, dann explorierst du danach;\n"
                "  nicht nach irgendwelchen festen Snake/Spiel-Vorgaben.\n\n"
                "### Delta-Erkennung (WICHTIG)\n"
                "- Lies die Dateien unter `.pi/work/{run_id}/{task_id}/project/`\n"
                "- Wenn dort bereits `index.html`, `style.css`, `app.js` existieren:\n"
                "  → Das ist eine **Aenderung** an einem bestehenden Projekt\n"
                "  → Beschreibe im Brief nur, was geaendert werden muss\n"
                "  → Beispiel: \"Contact-Sektion zu index.html hinzufuegen\" statt \"Erstelle eine Website\"\n"
                "- Wenn das Verzeichnis leer ist:\n"
                "  → Das ist ein **neues Projekt**\n"
                "  → Beschreibe die vollstaendige Anforderung\n\n"
                "### Read-only (HART)\n"
        ...
        if role == "PLANNER":
            parts.append(
                ...
                "**SEQUENCE: Fuer aufeinanderfolgende CODER-Aufgaben setze `dependencies` so, "
                "dass task-2 von task-1 abhängt (und task-3 von task-2). Nur so werden "
                "per-task Baselines zur richtigen Zeit erstellt. Ohne Dependency bleibt die "
                "Baseline auf dem Initialzustand und der Scope-Checker schlägt falsch an.\n\n"
            )
