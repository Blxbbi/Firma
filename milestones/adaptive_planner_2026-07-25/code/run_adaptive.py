"""
run_adaptive.py — ADAPTIVE PLANNER INTEGRATION (Meilenstein 2026-07-25)

WARUM DIESE DATEI?
------------------
run_pi_mesh.py war der urspruengliche Entrypoint fuer das Snake/Counter-Projekt.
Fuer die adaptive Planner-Integration brauchten wir einen sauberen, neuen Entrypoint
ohne die historischen Snake/Counter-Altlasten.

WAS WURDE GEAENDERT?
--------------------
1. ADAPTIVE PROMPTS (engine/transport/pi_mesh_transport.py)
   - RESEARCHER erkennt jetzt Delta vs. Neubau
   - PLANNER bekommt Delta-Erkennungs-Logik
   - Bei bestehendem Projekt: plane nur Aenderung (1-3 Tasks)
   - Bei neuem Projekt: plane vollstaendig (3-10 Tasks)

2. TASK-LIMIT ERHOEHT (engine/services/guardian.py)
   - Max 10 Tasks (vorher 5)
   - Kernel-Guards bleiben erhalten

3. CONFIG BEFREIT (diese Datei)
   - expected_artifacts: [] (Planner entscheidet selbst)
   - acceptance_criteria: [] (Planner definiert selbst)
   - Keine festen Vorgaben mehr

4. DELTA-ERKENNUNG
   - Wenn Projekt bereits Dateien enthaelt → nur Aenderung planen
   - Wenn Projekt leer ist → vollstaendig planen

TEST-ERGEBNIS (Run 8cfc323b):
-----------------------------
- Researcher: erkannte Delta (bestehendes Projekt mit index.html, style.css, app.js)
- Planner: erstellte 2 Tasks (statt bisher immer 3)
  * task-1: index.html (Contact-Sektion, CTA, SEO)
  * task-2: style.css (GitHub URL, Performance)
- Beide Tasks: COMPLETE in ~2 Minuten

NUTZUNG:
--------
export FIRMA_TRANSPORT=pimesh
export FIRMA_APP=counter
export FIRMA_PROJECT_DIR="pfad/zum/projekt"  # optional fuer Delta-Modus
python run_adaptive.py

ROLLBACK:
---------
Falls Probleme auftreten: export FIRMA_TRANSPORT=in-process
"""

"""
run_adaptive.py — Adaptive Planner-Integration fuer den Firma-Workflow.
...
class PiMeshReceiverLoop:
    ...
    def __init__(
        ...
        task_assigned_at: Optional[Dict[tuple, Tuple[str, float]]] = None,
    ):
    ...
    def _inline_plan_draft(self, crew_cwd: str, task_id: str, worker_payload: Dict[str, Any]) -> None:
        ...
        # Test hook: expand 3-task plans to 5 tasks to verify CODER gate serialization
        self._expand_plan_to_5_tasks(worker_payload)

    def _expand_plan_to_5_tasks(self, worker_payload: Dict[str, Any]) -> None:
        """Test hook: expand a 3-task plan to 5 tasks to verify CODER gate serialization.

        Splits task-3 (app.js) into 3 separate tasks:
        - task-3: mobile nav + smooth scrolling
        - task-4: intersection observer animations  
        - task-5: accessibility + keyboard nav
        """
        plan = worker_payload.get("plan_draft")
        if not isinstance(plan, dict):
            return
        tasks = plan.get("tasks", [])
        if len(tasks) != 3:
            return  # Only expand 3-task plans
        
        # Find task-3 (the last task)
        task_3 = None
        for t in tasks:
            if t.get("id") == "task-3":
                task_3 = t
        ...
        # Split task-3 into 3 tasks
        base_desc = task_3.get("description", "")
        base_artifacts = task_3.get("expected_artifacts", [])
        base_criteria = task_3.get("acceptance_criteria", [])
        
        # Create 3 new tasks from task-3
        task_3_new = {
            "id": "task-3",
            "description": base_desc + " (Part 1: mobile navigation toggle and smooth scrolling for anchor links)",
            "dependencies": ["task-2"],
            "expected_artifacts": base_artifacts,
            "acceptance_criteria": base_criteria[:3] if len(base_criteria) > 3 else base_criteria
        }
        task_4 = {
            "id": "task-4",
            "description": "Create app.js - Part 2: Intersection Observer for scroll-triggered fade-in animations with subtle reveal effects.",
            "dependencies": ["task-3"],
            "expected_artifacts": [{"path": "app.js", "type": "UPDATE"}],
            "acceptance_criteria": [
                "IntersectionObserver used for scroll-based animations",
                "Subtle fade-in effects on sections and cards",
                "No layout thrashing or performance issues"
            ]
        }
        task_5 = {
            "id": "task-5",
            "description": "Create app.js - Part 3: Accessibility enhancements including keyboard navigation, focus management, and ARIA live regions.",
            "dependencies": ["task-4"],
            "expected_artifacts": [{"path": "app.js", "type": "UPDATE"}],
            "acceptance_criteria": [
                "Keyboard navigation fully functional",
                "Focus management for mobile menu",
                "ARIA attributes and live regions implemented"
            ]
        }
        
        # Replace task-3 with task-3, task-4, task-5
        new_tasks = tasks[:2] + [task_3_new, task_4, task_5]
        plan["tasks"] = new_tasks
        worker_payload["plan_draft"] = plan
        logger.info(f"[Receiver] Expanded plan to 5 tasks (split task-3 into task-3/4/5)")
    ...
    def _has_agent_end(self, crew_cwd: str, task_id: str, assigned_at: Optional[float] = None) -> bool:
        log_path = self._worker_log_path(crew_cwd, task_id)
        if not log_path or not os.path.isfile(log_path):
            return False
        try:
            mtime = os.path.getmtime(log_path)
            if assigned_at is not None and mtime <= assigned_at:
                return False
    ...
    def _has_write_progress(self, crew_cwd: str, task_id: str, assigned_at: Optional[float] = None) -> bool:
        log_path = self._worker_log_path(crew_cwd, task_id)
        if not log_path or not os.path.isfile(log_path):
            return False
        try:
    ...
    def scan_once(self) -> List[Dict[str, Any]]:
        ...
        now = time.time()
        for key, (role, assigned_at) in list(self.task_assigned_at.items()):
            task_id = key[0] if isinstance(key, tuple) else key
            ...
            if os.path.isfile(resp_file):
                self.task_assigned_at.pop(key, None)
                continue

            fail = None
            if self._has_agent_end(crew_cwd, task_id, assigned_at):
                fail = (
                    'TASK_FAILED',
                    'WORKER_EXITED_WITHOUT_SUBMISSION: agent_end detected in worker.log without worker_response.',
                    role,
                )
            elif now - assigned_at > self.NO_PROGRESS_TIMEOUT_S and not self._has_write_progress(crew_cwd, task_id, assigned_at):
                fail = (
                    'WORKER_TIMEOUT',
...
def make_pimesh_messenger_callback(transport, provider, crew_cwds, project_root, models):
    ...
    async def _spawn_limited(role, payload, crew_cwd, model, session_id, session_dir, correction_note, spawn_key, task_assigned_at):
        ...
        if task_id is not None:
            task_assigned_at[(task_id, state_revision)] = (role, time.time())
...
# ---------------------------------------------------------------------------
# Kernel Bootstrap (gespiegelt von run_adaptive.main_logic, aber pimesh)
# ---------------------------------------------------------------------------
async def main_logic():
    ...
    logger.info("🚀 Starting Adaptive Run...")
    ...
    try:
        ...
        logger.info("✅ Adaptive Run finished.")
...
def _adaptive_config() -> Dict[str, Any]:
    prompt = (
        ...
        "PLANNER-HINWEIS: Du entscheidest selbst, wie viele Tasks gebraucht werden und welche Dateien erstellt werden. "
        "Analysiere die Anfrage und erstelle einen passenden Plan mit 1-10 Tasks. "
        "Jeder Task hat eine klare Beschreibung, Abhaengigkeiten, expected_artifacts und Akzeptanzkriterien. "
        "Halte dich an das Budget: max 10 Tasks, max 3 Dateien pro Task.\n\n"
