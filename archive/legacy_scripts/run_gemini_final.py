...
from engine.models import Project, Plan, Task, Run, RunStatus, ExecutionPhase, AssignedRole, TaskEvent, MessageType
...
async def setup_industrial_run(db_manager: DatabaseManager):
    """
    Bootstraps a completely isolated execution run.
    Sequence: Run -> Project -> Plan -> Task
    """
    async with db_manager.session_scope() as session:
        run_id = str(uuid.uuid4())
        run = Run(id=run_id, status=RunStatus.ACTIVE)
        session.add(run)
        await session.flush()
        ...
        logger.info(f"INDUSTRIAL BOOTSTRAP COMPLETE: Run={run_id}, Project={project.id}, Task={task.id}")
        return run_id, project.id, plan.id, task.id
...
async def worker_simulator(run_id, project_id, plan_id, task_id, orchestrator, transport, db_manager, planner, executor, prompt):
    worker_id = str(uuid.uuid4())[:8]
    logger.info(f"[Worker] Worker loop started (id={worker_id}) for run {run_id}")
        ...
        while orchestrator.running:
            try:
                ...
                if assignment.get("run_id") != run_id:
                    logger.warning(f"[Worker {worker_id}] Received assignment for wrong run {assignment.get('run_id')}. Skipping.")
                    continue
                ...
                try:
                    async with db_manager.session_scope() as session:
                        ...
                        result = await session.execute(select(Task).filter(Task.id == t_id, Task.run_id == run_id))
                        task = result.scalars().first()
                        
                        if not task:
                            logger.warning(f"[Worker {worker_id}] Task {t_id} not found in run {run_id}. Skipping.")
                            continue
                        ...
                        res_rev = await session.execute(select(Task.state_revision).filter(Task.id == task.id))
                        latest_revision = res_rev.scalar()
...
async def main():
    ...
    # FIX: Use a constant DB path for validation to avoid "DB-Drift"
    if os.path.exists("firma_industrial_validation.db"):
        os.remove("firma_industrial_validation.db")
        logger.info("Cleared old database for a clean industrial start.")
    
    db_path = "firma_industrial_validation.db"
    logger.info(f"Starting run with fixed validation database: {db_path}")
    ...
    # run_id is now the central context
    run_id, project_id, plan_id, task_id = await setup_industrial_run(db_manager)
    
    # Recovery is now run-scoped
    await scheduler.recover_stale_claims(run_id=run_id)
    ...
    logger.info(f"STARTING INDUSTRIAL RUN: {run_id}")

    print("\n+++ RUNNING GEMINI VERSION (INDUSTRIAL VALIDATION) +++\n")
    provider = OpenRouterProvider(
        api_key="REDACTED_OPENROUTER_KEY",
        model="google/gemini-flash-1.5-exp:free"
    )
    planner = PlannerWorker(provider, "google/gemini-flash-1.5-exp:free", dummy_mode=False)
    executor = ExecutionWorker(provider, "google/gemini-flash-1.5-exp:free", dummy_mode=False)

    orchestrator.running = True
    
    orchestrator_task = asyncio.create_task(orchestrator.run_forever())
    worker_task = asyncio.create_task(worker_simulator(run_id, project_id, plan_id, task_id, orchestrator, transport, db_manager, planner, executor, prompt))
