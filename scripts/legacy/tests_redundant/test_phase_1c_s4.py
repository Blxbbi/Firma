...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("kernel_phase1c_s4.log", mode='w'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("Phase1C-S4")
...
async def run_scenario_s4():
    """S4: Kernel crash post-CAS, pre-ack."""
    logger.info("--- SCENARIO S4: Kernel Crash Post-CAS, Pre-Ack ---")
    task_id = "T-S4"
    ...
    with db_manager.session_scope() as session:
        ...
        project = Project(id="P-S4", name="S4 Project")
        session.add(project)
        session.flush()
        plan = Plan(id="PL-S4", project_id=project.id, version=1, name="S4 Plan", status="IN_PROGRESS")
        session.add(plan)
        session.flush()
    ...
    with open("worker_s4.log", "w") as f:
    ...
    # 2. Kernel Crash post-CAS
    logger.info("Running Kernel with fault: crash_post_cas...")
    with open("kernel_s4_faulty.log", "w") as f:
        kernel_faulty = subprocess.Popen([sys.executable, "kernel_runner.py", "--fault", "crash_post_cas"], stdout=f, stderr=f)
    ...
    with open("kernel_s4_rec.log", "w") as f:
    ...
    if phase_post == ExecutionPhase.VERIFYING and rev_post == 1 and trans_post == 1:
        logger.info("✅ S4 SUCCESS: Kernel crash post-CAS handled correctly. Idempotency preserved.")
    else:
        logger.error(f"❌ S4 FAILURE: State={phase_post}, Rev={rev_post}, Trans={trans_post}")

async def main():
    try:
        await run_scenario_s4()
    except Exception as e:
        logger.exception(f"Critical Failure in S4: {e}")
